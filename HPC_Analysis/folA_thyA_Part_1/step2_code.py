...
def seq_find(seq, qscore, left, right, target_len, hamming_dist):
    """Identifies a target sequence using regular expressions. Sequence must be of specified length, 
    and gaps
...
for read_file in file_names:
    ...
    print(BC_sum)
    print(f'Total: {total}')
    both = [len(x) == 20 and len(y) == 20 and len(z) == 6 for x, y, z in zip(sgRNA1_dict[file_id], sgRNA2_dict[file_id], BC_dict[file_id])]
...
for read_file in file_names:
    ...
    for BC in ['TGAAAG', 'CATGAT', 'CCATGC', 'Other']:
...
for TP in pairwise_lib.keys():
    for BC in pairwise_lib[TP].keys():
        pd.DataFrame(pairwise_lib[TP][BC]).to_csv('BarcodedCounts/' + date + '_' + TP + '_' + BC + '_counts.csv')